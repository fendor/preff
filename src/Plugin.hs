...
import Constraint (Ct(..), Xi, CtLoc, ctEvidence, ctLocSpan, setCtLoc, setCtLocSpan, ctLoc, ctPred, mkNonCanonical)
import TcRnTypes (TcPlugin(..), TcPluginResult (..))
import TcPluginM (TcPluginM, lookupOrig, newFlexiTyVar, tcLookupTyCon, tcLookupClass, tcLookupId, newWanted, tcPluginIO, tcLookupDataCon, getTopEnv)
import OccName (mkTcOcc, mkDataOcc)
import Finder (findPluginModule, FindResult(..))
import Module (Module, mkModuleName)
import Outputable (empty, showSDocUnsafe, interppSP, ppr, parens, brackets, interpp'SP, Outputable, text, (<>), comma, space)
...
import TysWiredIn (promotedNilDataCon, promotedConsDataCon, promotedTupleDataCon, boolTy, promotedTrueDataCon, listTyCon, charTy, charDataCon)
...
import VarSet (pprVarSet, VarSet, elemVarSet, mkVarSet, emptyVarSet, unitVarSet, mapUnionVarSet, unionVarSets, disjointVarSet, unionVarSet)
...
removeSimplify analysis (OpConstraint OpAcceptableList [a, OpVar b, OpVar c])
  ...
  where indicesB = fromMaybe Set.empty (Map.lookup b (getHelper2 analysis))
...
removeSimplify' analysis (OpConstraintSet ls ch) =
  let t = ls >>= removeSimplify analysis in
  OpConstraintSet t (t /= ls || ch)
...
getInfo xs 
  ...
  where helper5 = getHelper5All xs
...
instance Prelude.Semigroup UnionFind where
  (<>) uf1 (UnionFind xs) = foldr add uf1 xs
    where add varSet (UnionFind uf)
            = case findIndex (\varSet' -> not (varSet `disjointVarSet` varSet')) uf of
                Nothing -> UnionFind $ varSet:uf
                Just i -> let (a, b:c) = splitAt i uf in
                          UnionFind $ a ++ (b `unionVarSet` varSet):c

instance Monoid UnionFind where
  mempty = UnionFind []

getUnionFindAll = constructTransform helper fold
  where helper (OpConstraint OpAcceptableList [OpVar a, OpVar b, OpVar c]) = UnionFind [mkVarSet [a, b, c]]
...
getHelper5All = constructTransform helper Map.unions
  where helper (OpConstraint OpEquality [p@(OpApp OpLookup [_, OpVar _, _]), OpVar m]) = Map.singleton m p
...
getHelper6All helper5 = constructTransform helper (Map.unionsWith maxFun)
  where helper (OpConstraint OpLeq [OpApp OpX [], OpApp OpLookup [_, OpVar xs, i]]) = helper' xs i OpX
...
getHelper8All helper5 = constructTransform helper concat
  where helper (OpConstraint OpEquality [OpApp OpDisequality [_, a, b], OpApp OpTrue []]) = [(a, b), (b, a)]
...
constructTransform transform union = mapUnion transform'
  where transform' (OpConstraintSet ys _) = mapUnion transform ys
...
getAnalysis xs = Analysis { getHelper = getHelperAll xs
...
getHelperAll = constructTransform helper (Map.unionsWith maxFun)
  where helper (OpConstraint OpLeq [OpApp OpX [], OpVar a]) = Map.singleton a OpX
...
getHelper2All = constructTransform helper (Map.unionsWith Set.union)
  where helper (OpConstraint OpLeq [OpApp OpX [], OpApp OpLookup [_, OpVar xs, i]]) = Map.singleton xs (Set.singleton i)
...
getHelper3All = constructTransform helper (Map.unionsWith maxFun)
  where helper (OpConstraint OpLeq [OpApp OpX [], OpApp OpLookup [_, OpVar xs, i]]) = Map.singleton (xs, i) OpX
...
getHelper4All = constructTransform helper (Map.unions)
  where helper (OpConstraint OpEquality [OpApp OpX [], OpApp OpLookup [_, OpVar xs, i]]) = Map.singleton (xs, i) OpX
...
freeVarsAll = constructTransform helper unionVarSets
  where helper (OpConstraint OpLeq [OpApp OpX [], OpVar _]) = emptyVarSet
